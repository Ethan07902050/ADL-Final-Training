...
FILE_RANGES = {
    ...
    "sgd_all": {"train": range(139, 140), "dev": range(1, 21), "test": range(1, 17)},
...
class SGDDataProcessor(object):
    ...
    def _create_examples_from_dialog(
        ...
        def format_dialog_id(dialog_id):
            ...
            id_mapping = {'MUL': 100, 'SNG': 101, 'PMUL': 102, 'SSNG': 103, 'WOZ': 104}
        ...
        for slot, values in slot_values.items():
            ...
            for slot_span in char_slot_spans:
                if slot_span["slot"] == slot:
                    ...
                    if "start" not in slot_span or "exclusive_end" not in slot_span:
                        # logging.info("cannot find key 'start' or 'exclusive_end'")
                        return {}

                    if slot_span["start"] not in alignments or slot_span["exclusive_end"] - 1 not in alignments:
                        return {}
